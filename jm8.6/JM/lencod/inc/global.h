...
{
  int                 type;           //!< type of syntax element for data part.语法元素的类型
  int                 value1;         //!< numerical value of syntax element语法元素的值
  int                 value2;         //!< for blocked symbols, e.g. run/level
  int                 len;            //!< length of code编码长度
...
#if TRACE
  #define             TRACESTRING_SIZE 100            //!< size of trace string字符串的大小
  char                tracestring[TRACESTRING_SIZE];  //!< trace string字符串
#endif

  //!< for mapping of syntaxElement to UVLC
  void    (*mapping)(int value1, int value2, int* len_ptr, int* info_ptr);//运用UVLC模式编码
...
typedef struct 
{
  int   no_slices;
  int   idr_flag;
  Slice *slices[MAXSLICEPERPICTURE];//一帧图片最大的片数
...
typedef struct
{
  int ProfileIDC;               //!< profile idc编码器的档次(baseline)主要档次(main)扩展档次(extended)
  int LevelIDC;                 //!< level idc   编码级别，大部分级别是与内存大小来分配的。解码器必须支持 

  int no_frames;                //!< number of frames to be encoded多少帧将会被编码
  int qp0;                      //!< QP of first frame第0帧的量化
  int qpN;                      //!< QP of remaining frames除了第零帧的量化
  ...
  int yuv_format;               //!< GH: YUV format (0=4:0:0, 1=4:2:0, 2=4:2:2, 3=4:4:4,currently only 4:2:0 is supported)  YUV格式的支持，当前只支持420格式的
  ...
  int  infile_header;           //!< If input file has a header set this to the length of the header 输入文件的头部长度
  ...
  int intra_period;             //!< Random Access period though intra I帧放置的周期

  int idr_enable;				//!< Encode intra slices as IDR使能片的IDR
  int start_frame;				//!< Encode sequence starting from Frame start_frame

  // B pictures
  // B 图像
  int successive_Bframe;        //!< number of B frames that will be used  P帧之间插入的B帧的个数 小于frameskip
  ...
  int StoredBPictures;           //!< Stored (Reference) B pictures replace P pictures (0: not used, 1: used)是否把B帧存储作为参考帧

  int symbol_mode;              //!< Specifies the mode the symbols are mapped on bits
  int of_mode;                  //!< Specifies the mode of the output file 输出文件模式 0 表示字节流 1 表示RTP流
  int partition_mode;           //!< Specifies the mode of data partitioning 是否支持数据分割，即每个slice的数据分成几个部分，0表示不进行分割 1-3表示分割的部分
  ...
  int pic_order_cnt_type;   // POC200301 视频采用了B帧，编码顺序和播放顺序不一致
...
typedef struct
{
  ...
  int framerate;
  int width;                   //!< Number of pels亮度图像宽度 像素单位
  int width_cr;                //!< Number of pels chroma 色彩宽度  是亮度的一半  像素单位
  int height;                  //!< Number of lines图像高度 像素单位
  int height_cr;               //!< Number of lines  chroma色彩图像高度 像素单位
  ...
  Macroblock    *mb_data;                                   //!< array containing all MBs of a whole frame 宏块指针
...
//帧场模式标识
  ...
  //delta_pic_order_always_zero_flag = 1 delta_pic_order_cnt[0]和[1]不在片头出现
  //=0上面的语法元素将会出现在片头
  //pic_order_cnt_type = 1时才会出现下面
  unsigned int delta_pic_order_always_zero_flag;
           int offset_for_non_ref_pic;//被用来计算非参考帧或场 picture order count
