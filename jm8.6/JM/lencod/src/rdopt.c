...
void init_rdopt ()
{
  get_mem_DCcoeff (&cofDC);//分配直流残差内存
  get_mem_ACcoeff (&cofAC);//分配交流残差内存
 ...
 */
 void encode_one_macroblock ()
 {
   ...
   int         intra1 = 0;
   //图像类型 img->type==I_SLICE 所以结果是 intra = 1
   int         intra       = (((img->type==P_SLICE||img->type==SP_SLICE) && img->mb_y==img->mb_y_upd && img->mb_y_upd!=img->mb_y_intra) || img->type==I_SLICE);

   //帧的类型
   int         spframe     = (img->type==SP_SLICE);//说明是SP帧
   int         siframe     = (img->type==SI_SLICE);//说明是SI帧
   int         bframe      = (img->type==B_SLICE); //说明是B 帧
  
   //此时RestrictRef = 0  是否将帧内刷新区域禁止再作为参考区域
   //runs = 1                              0              1                        0                  0                       0                         
   ...
   {//分析此路 帧方式fram
	...
	//快速编码使能
   if(input->FMEnable)
     decide_intrabk_SAD();//暂时不分析
   
   intra |= RandomIntra (img->current_mb_nr);    // Forced Pseudo-Random Intra

   //===== SET VALID MODES =====
   //设置有效模式
   valid[I4MB]   = 0;//采用4X4块 暂时不分析4X4块
   valid[I16MB]  = 1;//采用16X16块

   //valid[0] = 0;只有在P 或者 B 帧时才有效 所以0-7都是0
   valid[0]      = (!intra );
   ...
   valid[12]     = (siframe);//是否支持SI帧
/*
	enc_frame_picture->MbaffFrameFlag = img->MbaffFrameFlag = 
	(input->MbInterlace != FRAME_CODING);
