 ...
 *    get neighbouring positions. MB AFF is automatically used from img structure
 得到临近的位置 宏块自适应MB AFF
 * \param curr_mb_nr
 *   current macroblock number (decoding order) 当前宏块号
 * \param xN
 *    input x position X坐标
 * \param yN
 *    input y position Y坐标
 * \param luma
 *    1 if luma coding, 0 for chroma 1说明是亮度编码 0说明是色彩编码
 * \param pix
 *    returns position informations 返回位置信息
 ************************************************************************
 */
