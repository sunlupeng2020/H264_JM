...
void intrapred_luma_16x16()
{
  ...
  if (!(input->UseConstrainedIntraPred))
  {//帧间的宏块允许采用帧内预测方式得到  暂时分析这个部分
    up_avail   = up.available;
    left_avail = left[1].available;//左上角是否有效
  ...
  if (up_avail && left_avail)
  	//四舍五入求得像素的平均值，并将这个像素的值代替宏块每个像素的值
    s0=(s1+s2+16)/(2*MB_BLOCK_SIZE);             // no edge
  if (!up_avail && left_avail)
	//说明左侧预测值的平均值代表当前快的像素值
  	s0=(s2+8)/MB_BLOCK_SIZE;                     // upper edge
  if (up_avail && !left_avail)
  	//说明右侧预测值的平均值代表当前快的像素值
...
//将上面的垂直预测水平预测和直流预测
  ...
  //如果上 上左 和左 都无效 则直接返回 也就是说明这是第一个块
  ...
  {
    if (i<8)
		//16X16大小        8 - 6  9-5    14-0
		//H=(1-8求和)x *(P(7+x,-1)-P(7-x,-1))  
      ih += i*(imgY_pred[up.pos_y][up.pos_x+7+i] - imgY_pred[up.pos_y][up.pos_x+7-i]);
    else
		//8+7=15  -0 
		//9*(imgy_pred[y][x+15]-imgy_pred[-1(当前宏块0 ,0像素的左上面)][-1])
      ih += i*(imgY_pred[up.pos_y][up.pos_x+7+i] - imgY_pred[left[0].pos_y][left[0].pos_x]);

	//V=(1-8求和)y*(P(-1,7+y)-P(-1,7-y))
	//left[17];left[0]坐标是 -1，-1。   left[8+i]
