...
void code_a_picture(Picture *pic)
{
  ...
  /*
  初始化灵活排序FMO 默认采用的是type=0 交织模式  num_slice_groups_minus1=0 不采用FMO模式
	也就是说只有一个片
  */
  ...
  {
    // Encode one SLice Group  编码一个片组 FMO 灵活排序
    //FmoSliceGroupCompletelyCoded 灵活编码
    while (!FmoSliceGroupCompletelyCoded (SliceGroup))
    {
      // Encode the current slice
      //encode_one_slice编码一个片   return NumberOfCodedMBs;  返回编码个宏块数
	  //编码一片NumberOfCodedMBs
      NumberOfCodedMBs += encode_one_slice (SliceGroup, pic);
      FmoSetLastMacroblockInSlice (img->current_mb_nr);
      // Proceed to next slice
      img->current_slice_nr++;//当前片加一
      stat->bit_slice = 0;
    }
    // Proceed to next SliceGroup
    SliceGroup++;//编码号加1 开始编码下一个片组
  }

  //什么都不做
  FmoEndPicture ();//no thing
...
{
  static int prev_frame_no = 0; // POC200301 静态局部变量 先前的帧号 fram_num 初始化成0
  static int consecutive_non_reference_pictures = 0; // POC200301 连续的参考图像

#ifdef _LEAKYBUCKET_
  extern long Bit_Buffer[10000];
  extern unsigned long total_frame_buffer;//总共的帧缓冲区
#endif
...
#ifdef WIN32
  _ftime (&tstruct1);           // start time ms 记录下当前的时间
#else
  ftime (&tstruct1);
#endif
  time (&ltime1);               // start time s 获取当前时间

  //Rate control 码率暂时不分析
...
*/
  ...
  else
  {
    ...
    /*update the number of MBs in the basic unit for MB adaptive 
      ...
      rc_init_pict(1,0,1); 
      img->qp  = updateQuantizationParameter(0); //跟新码率 我们暂时不分析
...
*/
  ...
  /*
  	...
  	//这是开始配置整个帧的frame，poc(picture of count)
  */
...
//当前像素的
  ...
  //对每个图像的每个宏块进行操作 每个宏块的大小是16x16个像素
  ...
  {
  //不是B片 我们暂时只分析这个部分
  //(input->jumpd + 1) jumpd包括B帧和跳转帧 +1是I帧
  //每个P帧添加一定数量的B帧
  //start_tr_in_this_IGOP 初始化成0
  //#define IMG_NUMBER (img->number-start_frame_no_in_this_IGOP) 是当前的编码号
    img->tr = start_tr_in_this_IGOP + IMG_NUMBER * (input->jumpd + 1);//tr是对应的第i个参考帧
    ...
    if (IMG_NUMBER != 0 && input->successive_Bframe != 0)     // B pictures to encode
      nextP_tr_frm = img->tr;//B帧编码 暂时不分析 
    ...
    {//没有运用码率控制的情况下
      if (img->type == I_SLICE)
#ifdef _CHANGE_QP_
		//在大于qp2start 帧时就会采用QP02 量化参数
        if (input->qp2start > 0 && img->tr >= input->qp2start)
          img->qp = input->qp02;//超出qp2start时 I帧量化
        else
#endif    
		//否则的话还是使用原来的量化的参数
        img->qp = input->qp0;   // set quant. parameter for I-frame I帧量化输入量化28  运行这条语句
      else//如果img->type != I_SLICE如果 可能是P或者SP帧
      {
#ifdef _CHANGE_QP_
        if (input->qp2start > 0 && img->tr >= input->qp2start)
          img->qp = input->qpN2;//超出qp2start时 P帧量化
        else
#endif
          img->qp = input->qpN;//使用原先的量化值
        
        if (img->type == SP_SLICE)//=SP帧 可以暂时不分析SP帧
        {
          img->qp = input->qpsp;//SP帧的量化
          img->qpsp = input->qpsp_pred;//SP帧预测块的量化
        }   
      }
    }

	//img->mb_y_upd 在图像初始化的时候被初始化成=0
    img->mb_y_intra = img->mb_y_upd;  //  img->mb_y_intra indicates which GOB to intra code for this frame
	//通过增加帧内编码的宏块来提高容错能力 美N个帧有一个GOB宏块组采用帧内编码    
    if (input->intra_upd > 0) // if error robustness, find next GOB to update
    {//当前参考帧号/每隔一定帧增加一个帧内编码 = 一个帧内编码宏块的帧号?
    //帧Y方向 行号 添加帧内编码
    //intra_upd 暂时不分析提高容错能力 所以intra_upd=0
